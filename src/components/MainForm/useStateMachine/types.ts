...
export namespace Machine {
  export type Definition<
    ...
    HasContextSchema = Self extends { schema: { context: unknown } }
      ? true
      : false,
  > = {
    initial: A.IsUnknown<States> extends true
      ? LS.ConcatAll<
          [
      ...
      : [keyof States] extends [never]
        ...
        : keyof States;
    states: {
      [StateIdentifier in keyof States]: StateIdentifier extends A.String
    ...
    verbose?: boolean;
    console?: Console;
    $$internalIsConstraint?: A.CustomError<
  ...
  export namespace Definition {
    ...
    export type FromTypeParamter<D> = "$$internalIsConstraint" extends keyof D
      ? D extends infer X
    ...
    export type On<
      ...
      EventTypeConstraint = A.Get<
        EventsSchema,
        ExhaustiveIdentifier,
        false
      > extends true
        ? U.Exclude<keyof EventsSchema, ExhaustiveIdentifier>
        : A.String,
    > = {
      [EventType in keyof Self]: A.DoesExtend<EventType, A.String> extends false
        ? A.CustomError<
    ...
    export type Effect<D, P, StateValue = L.Pop<L.Popped<P>>> = (
      ...
      | void
      | ((
          parameter: EffectCleanupParameterForStateValue<D, StateValue>
        ) => void);

    type EffectImpl = (
      parameter: EffectParameter.Impl
    ) => void | ((parameter: EffectParameter.Cleanup.Impl) => void);
    ...
    > = {
      context?: A.DoesExtend<ContextSchema, { [$$t]: unknown }> extends false
        ? A.CustomError<
        ...
        : ContextSchema;
      events?: {
        [Type in keyof EventsSchema]: Type extends Definition.ExhaustiveIdentifier
          ? boolean
  ...
  export type Event<D, EventsSchema = A.Get<D, ["schema", "events"], {}>> =
    | O.Value<{
        [T in U.Exclude<
          keyof EventsSchema,
          Definition.ExhaustiveIdentifier
    ...
    | (A.Get<EventsSchema, Definition.ExhaustiveIdentifier, false> extends true
        ...
        :
              | (O.Value<{
                  [S in keyof A.Get<D, "states">]: keyof A.Get<
                    D,
                    ["states", S, "on"]
                  >;
              ...
              | (keyof A.Get<D, "on"> extends infer EventType
                  ? EventType extends any
                    ? { type: EventType }
                    : never
                  : never) extends infer InferredEvent
          ? InferredEvent extends any
            ? A.Get<InferredEvent, "type"> extends keyof EventsSchema
              ? never
              : A.Get<
                    InferredEvent,
                    "type"
                  > extends Definition.ExhaustiveIdentifier
                ? never
                : A.Get<
                      InferredEvent,
                      "type"
          ...
          type:
            | O.Value<{
                [S in keyof A.Get<D, "states">]: O.Value<{
                  [E in keyof A.Get<D, ["states", S, "on"]>]: A.Get<
                    D,
                    ["states", S, "on", E]
                  > extends infer T
                    ? (
                        T extends A.String ? T : A.Get<T, "target">
                      ) extends StateValue
                      ? E
                      : never
                    : never;
  ...
  export type ExitEventForStateValue<D, StateValue> = U.Extract<
    Event<D>,
    {
      type:
  ...
  export type Sendable<D, E = Event<D>> =
    | (E extends any
        ...
        : never)
    | E;
  type SendableImpl = Event.Impl["type"] | Event.Impl;
  ...
  export type SetContextImpl = (context: ContextUpdater.Impl) => {
    send: Send.Impl;
  };
...
export namespace L {
  ...
  export type Popped<A> = A extends []
    ? []
    : A extends [...infer X, any]
      ? X
      : never;
  export type Pop<A> = A extends []
    ? undefined
...
export namespace LS {
  export type ConcatAll<L> = L extends []
    ? []
    : L extends [infer H]
      ? H
      : L extends [infer H, ...infer T]
        ? `${S.Assert<H>}${S.Assert<ConcatAll<T>>}`
        : never;
}

export namespace S {
  export type Assert<T> = A.Cast<T, A.String>;
  export type IsLiteral<T> = T extends A.String
    ? A.String extends T
      ? false
      : true
    : false;
...
export namespace A {
  ...
  export type IsUnknown<T> = [T] extends [never]
    ? false
    : T extends unknown
      ? unknown extends T
        ? true
  ...
  export type IsPlainObject<T> = T extends A.Object
    ? T extends A.Function
      ? false
      : T extends A.Tuple
        ? false
  ...
  export type Get<T, P, F = undefined> = (
    P extends any[] ? _Get<T, P, F> : _Get<T, [P], F>
  ) extends infer X
    ? A.Cast<X, any>
    : never;
  ...
  export type CustomError<Error, Place> = Place extends (
    S.IsLiteral<Place> extends true ? Error : A.String
  )
  ...
  export const areEqual = <A, B>(_debug?: (value: A) => void) =>
    undefined as any as A.AreEqual<A, B>;
}
